...
void serviceMRequest()
{
	...
	count ++;
	// printf("[MMU] : Page reference : \ncount = %d | process id = %d | page number = %d\n",count,id,pageno);
	fprintf(resultf, "Page reference : (%d,%d,%d)\n", count, id, pageno);
	if (pcbptr[id].m < pageno || pageno < 0)
	{
		printf("[MMU] : Invalid Page Reference : \nProcessId= %d | Page Number = %d\n",id ,pageno);
		fprintf(resultf,"[MMU] : Invalid Page Reference : \nProcessId= %d | Page Number = %d\n",id ,pageno);
	...
	else
	{
		...
		{
			//PAGE FAULT
			printf("[MMU] : Page Fault occured\nProcess id = %d | Page No = %d\n",id,pageno);
		...
		{
			// sending signal to the scheduler that the empty frame is found
...
int main(int argc, char const *argv[])
{
	...
	printf("[MMU]: Page fault Count for each Process:\n");	
	fprintf(resultf,"[MMU]: Page fault Count for each Process:\n");
	printf("[Process Id]\t[Frequency]\n");
	fprintf(resultf,"[Process Id]\t[Frequency]\n");
	for(i = 0;i<k;i++)
	{
		printf("%d\t\t%d\n",i,pffreq[i]);
		fprintf(resultf,"%d\t\t\t%d\n",i,pffreq[i]);
