...
void clearResources() {
	if(shmctl(ptbid,IPC_RMID, NULL) == -1)	 perror("shmctl-ptb");
	if(shmctl(freelid,IPC_RMID, NULL) == -1) perror("shmctl-freel");
	if(shmctl(pcbid,IPC_RMID, NULL) == -1)  perror("shmctl-pcb");
	if(msgctl(readyid, IPC_RMID, NULL) == -1) perror("msgctl-ready");
	if(msgctl(msgq2id, IPC_RMID, NULL) == -1) perror("msgctl-msgq2");
	if(msgctl(msgq3id, IPC_RMID, NULL) == -1) perror("msgctl-msgq3");
}

void exit_after_clearing_resource(int status) {
	clearResources();
	exit(status);
}
...
void createFreeList() {
	...
	{	
		perror("freekey");
		exit_after_clearing_resource(EXIT_FAILURE);
	}
...
void createPCBs()
{
	...
	int allo_frame = 0;
	// printf("total pages = %d, number of process = %d, number of frames=  %d\n",totpages,k,f);
	int max = 0,maxi = 0;
	for(i=0;i<k;i++) {
		ptr[i].pid = -1;
...
int main(int argc, char const *argv[])
{
	...
	signal(SIGINT, exit_after_clearing_resource);
	if(argc < 4)
	{
		printf("master k m f\n");
		exit_after_clearing_resource(EXIT_FAILURE);
	...
	printf("[Master] : Process are being generated...\n");
	createProcesses();
	if(flag == 0) pause();
	clearResources();
	return 0;
