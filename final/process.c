...
int main(int argc, char *argv[]) //argv[] ={id,mq1,mq3,ref_string}
{
	...
	mq3_k = atoi(argv[3]);
	no_of_pages = 0;
	printf("[Process %d] : PAGE REFERENCE STRING = %s\n", id, argv[4]);
	conv_ref_pg_no(argv[4]);
	// printf("[Process %d] : PAGE REFERENCE STRING LENGTH = %d\n", id, no_of_pages);
	int mq1, mq3;
	...
	while (cpg < no_of_pages)
	{
		...
		printf("[Process %d] :  REQUESTING PAGE NUMBER %d\n", id, pg_no[cpg]);
		...
		{
			printf("[MMU | Process %d] : FREE FRAME = %d\n" , id, mmu_recv.frameno);
			cpg++;
		}
		...
		{
			printf("[Process %d] : INVALID PAGE REFERENCE STRING\n Treminating...\n", id) ;
			exit(1);
		}
	}
